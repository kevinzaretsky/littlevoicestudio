...
export async function POST(req: Request) {
  ...
  // Store the order in the database as "pending"
  await prisma.order.create({
    data: {
      stripeSession: session.id,
      amountTotal: line_items.reduce(
        (a: number, b: any) => a + b.price_data.unit_amount * (b.quantity || 1),
        0
      ),
      currency: 'eur',
      items,
